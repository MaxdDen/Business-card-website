...
- Jinja2 шаблонизация: `app/templates/base.html` с подключением Tailwind, светлая/тёмная тема через `data-theme` атрибут, с переключателем темы и навигацией.
...
## Этап 22 — исправление подсчета текстовых переменных в dashboard

- **Проблема:** В dashboard отображалось количество записей в таблице `texts_crm` (системные переводы CMS) вместо количества уникальных текстовых переменных из публичных шаблонов.
- **Исправление:** Изменена функция `get_dashboard_stats()` в `app/cms/routes.py`:
  - `texts_count` теперь подсчитывает уникальные комбинации `page.key` из таблицы `texts` (публичные шаблоны)
  - `languages_count` теперь берется из таблицы `texts` вместо `texts_crm`
  - Используется SQL-запрос `COUNT(DISTINCT page || '.' || key)` для подсчета уникальных переменных
- **Результат тестирования:** 
  - 30 уникальных текстовых переменных из публичных шаблонов (вместо 543 записей из `texts_crm`)
  - 3 языка (en, ru, ua) из таблицы `texts`
  - Корректное отображение статистики в dashboard
- **Тестирование:** Создан автотест `tests/auto_tests/test_dashboard_texts_count_fix.py` для проверки исправления
- **Интеграция:** Тест добавлен в `manage_tests.py` как `dashboard_texts_count_fix`

**Результат:** Dashboard теперь корректно отображает количество текстовых переменных из публичных шаблонов, а не системных переводов CMS.
